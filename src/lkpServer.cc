...

{
    ...
    server_.setConnectionCallback(
        std::bind(&lkpServer::onConnection, this, std::placeholders::_1));

    //绑定lkpCodec接收server新消息的回调函数，解析后形成正确类型的message
    server_.setMessageCallback(
        std::bind(&lkpCodec::onMessage, &codec_, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
